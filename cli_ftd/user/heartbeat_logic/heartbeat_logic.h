...
uint8_t writeByte_3(uint8_t, uint8_t, uint8_t, uint16_t);
...
// This function handles all read commands or stated another way, all
// information requests. It starts a request by writing the family byte, an
// index byte, and a write byte and then then delays 60 microseconds, during
// which the MAX32664 retrieves the requested information. An I-squared-C
// request is then issued, and the information is read. This differs from the
// above read commands in that it returns a 16 bit integer instead of 8.
...
// which the MAX32664 retrieves the requested information. An I-squared-C
// request is then issued, and the information is read. This differs from the
// above read commands in that it returns a 4 byte (uint32_t) integer instead
// of 8.
...
// index byte, and a write byte and then then delays 60 microseconds, during
// which the MAX32664 retrieves the requested information. An I-squared-C
// request is then issued, and the information is read. This function is very
// similar to the one above except it returns three uint32_t bytes instead of
// one.
...
// Family Byte: READ_DEVICE_MODE (0x02) Index Byte: 0x00, Write Byte: 0x00
// The following function puts the MAX32664 into bootloader mode. To place the
// MAX32664 into bootloader mode, the MFIO pin must be pulled LOW while the
// board is held in reset for 10ms. After 50 addtional ms have elapsed the board
// should be in bootloader mode and will return two bytes, the first 0x00 is a
...
// This function sets very basic settings to get LED count values from the
// MAX30101. Sensor data includes 24 bit LED values for the three LED channels:
// Red, IR, and Green.
...
// the RED, IR, and GREEN LEDs. In addition it gets the 8 bytes from the FIFO
// buffer related to the wrist heart rate algortihm: heart rate (uint16_t),
// confidence (uint8_t), SpO2 (uint16_t), and the finger detected status
// (uint8_t). Note that the the algorithm is stated as "wrist" though the sensor
// only works with the finger. The data is loaded into the whrmFifo and
// returned.
struct bioData readSensor();
...
// Family Byte: ENABLE_SENSOR (0x44), Index Byte: ENABLE_ACCELEROMETER (0x04),
// Write Byte: accelSwitch (parameter - 0x00 or 0x01). This function enables the
// ACCELEROMETER.
...
// Family Byte: WRITE_REGISTER (0x40), Index Byte: WRITE_MAX30101 (0x03), Write
// Bytes: Register Address and Register Value This function writes the given
// register value at the given register address for the MAX30101 sensor and
// returns a boolean indicating a successful or non-successful write.
void writeRegisterMAX30101(uint8_t, uint8_t);

// Family Byte: WRITE_REGISTER (0x40), Index Byte: WRITE_ACCELEROMETER (0x04),
// Write Bytes: Register Address and Register Value This function writes the
// given register value at the given register address for the Accelerometer and
// returns a boolean indicating a successful or non-successful write.
void writeRegisterAccel(uint8_t, uint8_t);

// Family Byte: READ_REGISTER (0x41), Index Byte: READ_MAX30101 (0x03), Write
// Byte: Register Address This function reads the given register address for the
// MAX30101 Sensor and returns the values at that register.
...
// Family Byte: READ_ATTRIBUTES_AFE (0x42), Index Byte: RETRIEVE_AFE_MAX30101/
// (0x03) This function retrieves the attributes of the AFE (Analog Front End)
// of the MAX30101 sensor. It returns the number of bytes in a word for the
// sensor and the number of registers available.
struct sensorAttr getAfeAttributesMAX30101();
...
struct sensorAttr getAfeAttributesAccelerometer();
...
// Family Byte: DUMP_REGISTERS (0x43), Index Byte: DUMP_REGISTER_ACCELEROMETER
// (0x04) This function returns all registers and register values sequentially
// of the Accelerometer: register zero and register value zero to register n and
// register value n.
...
// of the full-scale ADC range that the Automatic Gain Control algorithm is
// using.
...
// This function returns the sensitivity (percentage) of the automatic gain
// control.
